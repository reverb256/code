      ...
      <Flex direction="column" gap="4">
        ...
        <Box>
          ...
          <Text size="1" className="mt-1 block text-gray-11">
            API usage and estimated costs from Claude Code
          </Text>
        </Box>
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          <SummaryCard
            ...
            value={`$${fmtDollars(totalUsage.totalCost)}`}
          ...
          <SummaryCard
            ...
            iconBg="bg-purple-3"
            label="Total Tokens"
            value={fmtTokens(totalUsage.totalTokens)}
            sub={`${fmtTokens(totalUsage.totalInput)} in / ${fmtTokens(totalUsage.totalOutput)} out`}
          />
          <SummaryCard
            icon={<Pulse size={18} className="text-blue-11" />}
            iconBg="bg-blue-3"
            label="Cache Savings"
            value={fmtTokens(totalUsage.totalCacheRead)}
                  ...
                  >
                    ...
                    >
                      ...
                      <div
                        ...
                        title={`${item.label}: $${fmtDollars(item.cost)}`}
                      />
                    </Flex>
                    <Text
                ...
                return (
                  ...
                  >
                    <div className="mb-2 flex items-center justify-between">
                      <span className={`font-medium text-${colorClass}-11`}>
                        {model.displayName}
                      </span>
                      <span className="font-bold text-green-11">
                        ${fmtDollars(model.cost)}
                      </span>
                    </div>
                    <div className="grid grid-cols-2 gap-2 text-[12px]">
                      <Flex justify="between">
                        <Text className="text-gray-11">Input</Text>
                        <Text>{fmtTokens(model.inputTokens)}</Text>
                      </Flex>
                      <Flex justify="between">
                        <Text className="text-gray-11">Cache Read</Text>
                        <Text>{fmtTokens(model.cacheReadTokens)}</Text>
                      </Flex>
                      <Flex justify="between">
                        <Text className="text-gray-11">Output</Text>
                        <Text>{fmtTokens(model.outputTokens)}</Text>
                      </Flex>
                      <Flex justify="between">
                        <Text className="text-gray-11">Cache Write</Text>
                        <Text>{fmtTokens(model.cacheWriteTokens)}</Text>
                      </Flex>
                    </div>
                    ...
                    >
                      ...
                      >
                        <Text className="mb-1 font-mono text-[11px] text-gray-11">
                          {day.messageCount.toLocaleString("en-US")}
                        </Text>
                        <div
          ...
          <Card>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <StatBox
                ...
                value={(todayActivity?.sessionCount ?? 0).toLocaleString(
                  "en-US",
                )}
              />
              <StatBox
                label="Recent Tool Calls"
                value={(todayActivity?.toolCallCount ?? 0).toLocaleString(
                  "en-US",
                )}
              />
            </div>
        ...
        <Card>
          ...
          <Text size="1" className="mb-3 block text-gray-11">
            Estimated costs at API retail rates (not actual charges for
            subscription plans)
          </Text>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-gray-6 border-b">
                  <th className="px-2 py-2 text-left font-medium text-gray-11">
                    Model
                  </th>
                  <th className="px-2 py-2 text-right font-medium text-gray-11">
                    Input
                  </th>
                  <th className="px-2 py-2 text-right font-medium text-gray-11">
                    Output
  ...
  return (
    <Box className="rounded border border-gray-6 bg-gray-2 p-3">
      <Flex align="center" gap="2" mb="2">
        ...
        <Box>
          <Text size="1" className="block text-gray-11">
            {label}
          </Text>
          <Text size="4" weight="bold" className={`block ${valueColor ?? ""}`}>
            {value}
          </Text>
        </Box>
      </Flex>
